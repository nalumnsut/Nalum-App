...
import { Card, Screen } from "@/components/ui/nalum";
  ...
  return (
    <Card>
      ...
      <Text className="mt-1 text-muted">
        {p?.currentRole ?? "NSUT community member"}
        {p?.currentCompany ? ` · ${p.currentCompany}` : ""}
      </Text>
      <Text className="mt-2 font-mono text-xs text-maroon">
        {p ? `${p.batch} · ${p.branch} · ${p.campus}` : user.role}
      </Text>
    </Card>
  );
}
export default function Directory() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => usersApi({ q: q || undefined, limit: 20, offset: 0 }),
  });
  return (
    <Screen>
      ...
      <TextInput
        ...
        placeholder="Search people, companies, or cities"
      ...
      {isLoading ? (
        ...
        <Pressable onPress={() => refetch()}>
          <Text className="text-maroon">
            Could not load directory. Tap to retry.
          </Text>
        </Pressable>
      ) : (
        <FlashList
